...
## The Eye of Thundera — the vendored Inspector, deleted
The QA sweep now comes from a package instead of a copy. `inspector.py` (193
lines) existed here, byte-identical, in `ember-lite` and `ember-pro`, and as a
diverged seven-module ancestor in `fft-psx-vera` — four copies, every fix
landing in exactly one. It was extracted to `github.com/xsytrance/eyeofthundera`
(installable `eye-of-thundera`, CLI `thundera`); undertale-vera is its first
consumer, which is the step that makes the extraction worth anything.

- **Deleted** `inspector.py`. **Added** `thundera.toml` (the only
  undertale-vera-specific part) and `requirements-qa.txt` (optional; nothing in
  CI installs it). `.thundera/` run output is gitignored.
- **Coverage went up, not down.** The old inspector knew five paths and no views:
  it never saw `soundtest`, `workshop`, `guided`, `commons`, `credits`,
  `multivera`, `howitworks` or `saves`, because they have no URL — this app has
  no routing, and views switch on nav clicks. The config expresses that with
  `pre_clicks`, so 13 surfaces × 2 viewports = 26 page views now get looked at,
  against the old 5 paths. On top of status codes it checks contrast, overflow,
  clipped text, misalignment, tap targets, console errors and failed requests.
- **The `REQUIRED_ASSETS` list is gone on purpose.** A hand-maintained list of
  three files drifts; the browser engine records every request the real page
  makes, so a missing `app.js` or stylesheet is now a `network` finding on
  `chat`. The four `/api/*` surfaces were carried over as-is.
- **`tools/frontend_smoke.py` stays.** It looked like a second candidate for
  deletion and is not one: it asserts what the app *does* (a fixture uploads,
  the route badge reads Pacifist, chat replies, the Deltarune save flips the
  Dark World, the roster reseats, an egg fires). The Eye judges how a page
  *looks* and has no notion of a content assertion. Replacing one with the other
  would have quietly dropped the front end's only functional merge gate.
- First run against :9092 — **26 page views, 0 errors, 0 skipped, 57 warnings**
  (10 clean), every warning a mobile layout nit: 42 tap-target, 15 contrast, and
  soundtest at 390px the worst surface at 15. `--engine http` verified as the
  no-browser fallback: 13 surfaces, all 200.
- `pytest -q` → **367 passing**, unchanged: no test ever referenced the
  inspector and CI never ran it, so the deletion touches nothing under test.
  The two CI gates (`syntax + pytest`, `frontend smoke`) are untouched.

## Clearing what the Eye found
Every warning the Eye of Thundera raised against the app is now fixed: **57 → 7**,
and all 7 survivors are the tool's own false positives (below), not app defects.
Verified by re-running the sweep against the recorded baseline: **51 fixed, 1 new**
(and the "new" one is another instance of the same artifact, surfaced because the
layout shifted a different element under the bottom bar).

**The real bug — the current view's nav label was unreadable.** `undertale.css`
had `.navitem.sel { background:#fff; color:#000 }` (the skin's menu-feel
inversion) and then, five lines later, `.navitem:hover { background:#1a1a1a }`.
Same specificity, both `!important`, hover declared later — so hover won and left
**black text on #1a1a1a, 1.2:1**. Since the pointer normally rests on the item
you just clicked, the label of the view you were on was invisible most of the
time. It showed on all 7 desktop views. Fixed by scoping the hover away from the
selected item: `.navitem:not(.sel):hover`.

**42 tap-target warnings → 0.** Almost every control sat at 27–28px, just under
the 32px floor at which a control counts as cramped for a fingertip. One
mobile-only block in `determination.css` sets a 36px floor (not 32 — headroom, so
a later padding tweak can't silently push them back under) on `.btn`, `.st-mode`,
`.save-pill` and `.topbar-modes`, plus `min-width` for the single-glyph transport
buttons. Desktop is untouched: this is pointer precision, not layout.
- The `↻` on the quote and lore cards was the worst at **23×19.2px**, and it
  appears on all 9 views — 18 of the 42. It is absolutely positioned, so it now
  centres its glyph in a 36px box pulled up into the blank corner beside the
  card label, costing no layout.
- `#credits-support` ("tip jar ♥") is inline inside a sentence, so it got padding
  rather than a 36px box, which would have gapped the prose. First attempt failed
  silently: `.linklike` is re-declared with `padding: 0` further down the file, so
  the base rule won on order — **the same trap as the nav bug**. Fixed with
  `button.linklike`.

**The 7 remaining warnings are not ours.** All are contrast findings reporting a
background of `rgb(255,255,255)` on pages that are black with legible white text
— confirmed against the screenshots. The Eye composites backgrounds at scroll 0
via `elementsFromPoint`, so text sitting *below the fold* under the fixed white
bottom nav is judged against the **bar's** background instead of the page's. A
perfect 1:1 same-colour ratio is the tell. Logged upstream in the Eye's HANDOFF
under known gaps; fixing it properly is a design decision there, because the same
mechanism is what legitimately catches text behind a translucent sticky header.

Verified: `pytest -q` → **367 passing**; `tools/frontend_smoke.py` → **PASSED**
(run against a throwaway instance on :9098 with its own DB, so no dev data was
touched); both stylesheets brace-balanced. No JS or Python changed.
