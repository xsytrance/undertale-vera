...
- `power_config.py` — the power ladder (🕯 none/Spark, 🔑 openrouter BYOK, 🖥 ollama,
  ☁ anthropic), UI-selectable via `/api/power`; persists in gitignored
  `ember_power.json` (chmod 600, key only ever returned masked); config wins over env.
- `spark.py` — the model-less chat voice (intent ladder + per-character voice packs;
