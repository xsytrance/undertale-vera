...
def test_real_completed_save_full_promotions():
    ...
    assert f["name"] == "TESTER" and c["name"] == "high"
...
def test_dr_ini_parse_and_upload_corroboration():
    ...
    assert ini["name"] == "TESTER" and ini["uraboss"] == 2 and ini["love"] == 1
    r = client.post("/api/upload", files={
...
def test_area_never_guessed_for_deltarune():
    import save_flavor
    t = build_deltarune_truth(dp.parse_deltarune_save(_read("filech1_0_completed"), "filech1_0"))
    assert save_flavor.area_from_save(t) is None   # Undertale's area table must not label DR rooms
