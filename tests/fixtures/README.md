...
## Deltarune fixtures (regenerated synthetic, July 2026)

`filech1_0`, `filech1_0_early`, `filech1_0_completed`, and `dr_uraboss.ini` are
**fully synthetic**, generated by `tools/make_synthetic_fixtures.py`: the
corroborated Chapter 1 layout length with the mapped evidence lines set to
test values and every other line zeroed. The player name is `TESTER`. Earlier
versions of two of these files derived from a real save corpus; nothing from
any real player's save remains in the repository.
